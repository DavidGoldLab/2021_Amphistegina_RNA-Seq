...
if (file.exists("4_Photosynthesis.Heat_Map.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("4_Photosynthesis.Heat_Map.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("3_Gene_Matrix.txt", header=T, com='', row.names=1, check.names=F, sep='\t')
...
write.table(data, file="4_Photosynthesis.Heat_Map.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
heatmap_data = data
pdf("4_Photosynthesis.Heat_Map.log2.centered.genes_vs_samples_heatmap.pdf")
...
", "4_Photosynthesis.Heat_Map.log2.centered" ) )
dev.off()
